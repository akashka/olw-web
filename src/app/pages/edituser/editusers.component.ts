    ...
    this.authService.searchUser().then((result) => {
      ...
      for (var i = 0; i < this.users.length; i++) {
        if (this.users[i]._id == params) {
          ...
          this.confirm_password = this.users[i].password;
          this.name = this.users[i].name;
          this.center = this.users[i].center;
          this.active = this.users[i].active;
          this.isPasswordMatching = true;
          this.isExisting = false;
          this._id = this.users[i]._id;
          this.existingUser = false;
        }
      }
      this.loader = false;
  ...
  save() {
    if (!this.isExisting && this.isPasswordMatching) {
      ...
      this.authService.updateAccount(details).then((result) => {
        this.showToast(NbToastStatus.DANGER, 'Success!', 'User is Updated Successfully');
        this.loader = false;
